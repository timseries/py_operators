...
class GroupAverage(Average):
    ...
    def create_csr_avg(self,ws_mcand):
        ws_mcand.flatten()
        #determine the size (g_i) of each element, used in average energy computation
        g_i=ws_mcand.is_wavelet_complex()+1
        ...
        csr_data=1.0/(g_i*self.groupsize*np.ones(csr_rows.size,))
